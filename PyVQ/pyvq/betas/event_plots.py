...
def event_stress_movie(
        model,
        event_output_text_file,
        savefile,
        plotting,
        FPS=3,
        DPI=100):

    event_data = np.genfromtxt(
        event_output_text_file,
        dtype=[
            ('sweep_num',
             'int'),
    ...
    triggerSecElements = [id for id in range(
        model.num_elements()) if model.element(id).section_id() == sectionID]
    ...
    plt.figtext(
        0.97,
        0.6,
        '{} [units are mean stress drop]'.format(plotting),
        rotation='vertical')
    ...
    for id in triggerSecElements:
        mean_rake += model.element(id).rake() / len(triggerSecElements)
    ...
    with writer.saving(fig, savefile, DPI):
        ...
        for sweep_num in range(num_sweeps):
            ...
            # bottom.
            this_sweep = event_data[
                np.where(event_data['sweep_num'] == sweep_num)]
            for row in this_sweep:
                ...
                if model.element(ele_id).section_id() == sectionID:
                    ...
                    element_grid[grid_row, grid_col] = row[
                        plotting] / np.abs(row['stress_drop'])
                else:
                    sys.stdout.write(
