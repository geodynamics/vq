...
def quick_figs(
    ...
    with h5py.File(vc_data_file, 'r') as vc_data:
        ...
        if events_start is None:
            events_start = 0
        if events_end is None:
            events_end = len(events) - 1
        ...
        #
        print "... and bigmags "
        big_mags = [[rw['event_year'], rw['event_magnitude']]
                    for rw in events if rw['event_magnitude'] >= m0]
        big_mag_dts = [[rw[0], rw[0] - big_mags[j][0]]
                       for j, rw in enumerate(big_mags[1:])]
        #
        ...
        ax.plot(*zip(*[[m, len(mags) - j] for j, m in enumerate(mags)]),
                color='b', marker='.', ls='-', zorder=4, label='Cumulative $N(>m)$')
        # and the pdf...
        dolog = True
        ax.hist(
            mags,
        ...
        ax.plot(T[1:], zip(*dts)[1], marker='.', ls='-',
                color='b', zorder=8, label='$dt(t)$')
        ave_len = 100
        print "plot mean intervals over %d intervals(%d events).(%d)" % (ave_len, ave_len + 1, len(figs))
        ax.plot(T[ave_len:],
                [(t - T[j]) / float(ave_len) for j,
                 t in enumerate(T[ave_len:])],
                color='c',
                lw=2,
                zorder=11,
                label='$<dt(t)>_{%d}$' % ave_len)
        # set up dt range:
        ...
        ax_mags.vlines(*(zip(*big_mags)),
                       ymax=[3.0 for x in big_mags],
                       color='r',
                       lw=1.25,
                       zorder=2,
                       label='m>%.2f' % m0)
        ...
        ax.plot(zip(*big_mag_dts)[0],
                zip(*big_mag_dts)[1],
                'g.-',
                zorder=7,
                lw=1.5,
                label='$m>%.2f intervals')
        ax_mags = ax.twinx()
        ax_mags.vlines(*(zip(*big_mags)),
                       ymax=[3.0 for x in big_mags],
                       color='m',
        ...
        if normed:
            N = 1.0
        ax.plot([.5 * (x + h_cum[1][j]) for j,
                 x in enumerate(h_cum[1][1:])],
                [N - x for x in h_cum[0]],
                'c-')
...
class Sweep(object):
    ...
    def __init__(
            ...
            vc_data_file=default_events,
            block_id=None):
        self.sweep_sequences = sweep_sequence(
            event_number=event_number,
            block_id=block_id,
            vc_data_file=vc_data_file)
        self.shear_stress_sequences = shear_stress_sequence(
            sweepses=self.sweep_sequences, do_print=False)
        #
    ...
    def plot_slips(self, block_ids=None, fignum=0):
        ...
        for block_id in block_ids:
            rws = numpy.core.records.fromarrays(
                zip(
                    *
                    filter(
                        lambda x: x['block_id'] == block_id,
                        self.shear_stress_sequences)),
                dtype=self.shear_stress_sequences.dtype)
            plt.plot(
                rws['sweep_number'],
                rws['block_slip'],
                '.-',
    ...
    def plot_stress_drop(self, block_ids=None, fignum=0):
        ...
        for block_id in block_ids:
            ...
            plt.plot(
                ...
                label='block_id: %d' %
                block_id)
        plt.plot([min(self.shear_stress_sequences['sweep_number']), max(
            self.shear_stress_sequences['sweep_number'])], [0., 0.], 'k-')
        plt.legend(loc=0, numpoints=1)
    ...
    def plot_stress(self, block_ids=None, fignum=0):
        ...
        for block_id in block_ids:
            ...
            plt.plot(
                ...
                block_id)
            plt.figure(fignum)
        ax0.plot([min(self.shear_stress_sequences['sweep_number']), max(
            self.shear_stress_sequences['sweep_number'])], [0., 0.], 'k-')
...
def shear_stress_sequence(
        ...
        sweepses=None):
    if sweepses is None:
        sweepses = sweep_sequence(
            block_id=block_id,
            event_number=event_number,
            vc_data_file=vc_data_file)
    #
    outsies = [[rw['sweep_number'],
                rw['block_id'],
                rw['block_slip'],
                rw['shear_init'],
                rw['shear_final'],
                rw['shear_init'] - rw['shear_final'],
                (rw['shear_init'] - rw['shear_final']) / rw['shear_final']] for rw in sweepses]
    #
    if do_print:
        for rw in outsies:
            print rw
    ...
    return numpy.core.records.fromarrays(
        zip(*outsies), names=cols, formats=[type(x).__name__ for x in outsies[0]])
#
...
def sweep_sequence(event_number=0, block_id=None, vc_data_file=default_events):
    ...
    with h5py.File(vc_data_file) as vc_data:
        sweep_range = [
            vc_data['events'][event_number]['start_sweep_rec'],
            vc_data['events'][event_number]['end_sweep_rec']]
    ...
    if block_id is not None:
        ...
        sweeps = numpy.core.records.fromarrays(
            zip(*filter(lambda x: x['block_id'] == block_id, sweeps)), dtype=d_type)
...
def get_h5_col(col_name, vc_data_file=default_events):
    ...
    if isinstance(col_name, str):
        col_name = [col_name]
    if col_name[0] not in ('events', 'sweeps'):
        col_name.insert(0, 'events')
    #
    with h5py.File(vc_data_file) as vc_data:
