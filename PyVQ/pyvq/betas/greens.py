...
class PyGreens(object):
    ...
    def __init__(
            self,
            greens_fname='model1_greens_3000.h5',
            do_shear=True,
            do_normal=False):
        '''
        ...
        #
        if not isinstance(greens_fname, str):
            return None
        if not os.path.isfile(greens_fname):
            return None
        #
        if do_shear:
            self.ary_shear = get_h5_greens_array(
                greens_fname=greens_fname, shear_normal='shear')
        if do_normal:
            self.ary_normal = get_h5_greens_array(
                greens_fname=greens_fname, shear_normal='normal')
        #
    #
    ...
    def get_shear_h5(self, greens_fname=None):
        if greens_fname is None:
            greens_fname = self.greens_fname
        return get_h5_greens_array(
            greens_fname=greens_fname,
            shear_normal='shear')
    ...
    def plot_greens(
        ...
        '''
        #
        if greens_ary is None:
            shear_normal = shear_normal_aliases(shear_normal)
            if shear_normal == 'greens_shear':
                g_data = self.get_shear()
            if shear_normal == 'greens_normal':
                g_data = self.get_normal()
        #
        sh_0 = g_data.shape
        ...
        #
            ...
            ax.plot([x + 1 for x in xrange(len(g_data[0]))],
                    sorted(g_data[0]), '.-')
        #
        del(g_data)
    #
    # note: another way to do this is to make a sub-class like greens_hist(object), and then child classes:
    # plot_normal_hist(greens_hist), in which shear_normal='normal' is defined
    # in the child objects __init__() function.

    def plot_normal_hist(
            self,
            greens_ary=None,
            fnum=0,
            do_clf=True,
            n_bins=1000,
            **hist_kwargs):
        return self.plot_greens_hist(
            shear_normal='normal',
            greens_ary=greens_ary,
            fnum=fnum,
            do_clf=do_clf,
            n_bins=n_bins,
            **hist_kwargs)
    ...
    def plot_greens_hist(
        ...
        if greens_ary is None:
            ...
            if shear_normal == 'greens_shear':
                greens_ary = self.get_shear()
            if shear_normal == 'greens_normal':
                greens_ary = self.get_normal()
        #
        ...
        gr_val_max_abs, gr_val_min_abs = max(
            abs(greens_ary[0])), min(abs(greens_ary[0]))
        ...
        if do_fit:
            try:
                ...
                # contains the left edges + right edge of final entry.
                bin_edges = gr_hist[1]
                bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2.
                #
                x_hist, y_hist = zip(
                    *[[x, math.log10(y)] for x, y in zip(bin_centers, gr_hist[0]) if y > 0])
                #
                # plt.figure(fnum+1)
                # plt.clf()
                ...
                # plt.figure(0)
                # because we treat A like --> 10**log(a), for linearization.,
                # so note this is log(log(y))...
                gauss_p0 = [math.log10(max(y_hist)), 0., 1.0]
                # now, guess sigma:
                ...
                coeff, var_matrix = scipy.optimize.curve_fit(
                    gauss_pdf, x_hist, y_hist, p0=gauss_p0)
                #
                print "fit complete: A, mu, sigma = ", coeff, gauss_p0
                #
                x_hist_fit = numpy.arange(min(x_hist), max(
                    x_hist), .5 * (max(x_hist) - min(x_hist)) / float(n_bins))
...
def gauss_fit_mc(
    ...
    #if len(p0)<4: p0+=[0.]
    #
    fitfunc = lambda p, x: (10.**p[0]) * \
        exp(-0.5 * ((x - p[1]) / p[2])**2) + p[3]
...
def cap_greens(
    ...
    if isinstance(top_n, float):
        top_n = min(1, int((1.0 - top_n) * g_data.size))
    if isinstance(bottom_n, float):
        bottom_n = int((1.0 - bottom_n) * g_data.size)
    #
    sh_0 = g_data.shape
    g_data.shape = (1, g_data.size)
    g_data[0].sort()
    ...
    with h5py.File(greens_fname, 'r+') as gr_file:
        ...
        # for j,k in itertools.product(xrange(gr_data.shape[0]),
        # xrange(gr_data.shape[1])):
        for j, k in itertools.product(
            xrange(
                g_data.shape[0]), xrange(
                g_data.shape[1])):
...
def plot_greens_hists(
    ...
    with h5py.File(greens_fname, 'r') as f:
        ...
        gr_hist_diag = obj_gr.plot_greens_hist(
            shear_normal=None,
            greens_ary=numpy.array(
                [
                    x for k,
                    rw in enumerate(
                        f[shear_normal]) for j,
                    x in enumerate(rw) if j == k]),
            fnum=fnum,
            do_clf=do_clf,
            n_bins=n_bins,
            do_fit=False,
    ...
    with h5py.File(greens_fname, 'r') as f:
        ...
        gr_hist_offdiag = obj_gr.plot_greens_hist(
            shear_normal=None,
            greens_ary=numpy.array(
                [
                    x for k,
...
def plot_greens_hist(
    ...
    #
    obj_gr = PyGreens(
        greens_fname=greens_fname, do_shear=(
            str_shr_norm == 'greens_shear'), do_normal=(
            str_shr_norm == 'greens_normal'))
    # return obj_gr
    # change this to allow normal plots too:
    gr_hist = obj_gr.plot_shear_hist(
        greens_ary=greens_ary,
        fnum=fnum,
        do_clf=do_clf,
        n_bins=n_bins,
...
def greens_consistency_check(
    ...
    with h5py.File(greens_fname) as gr_data:
        ...
        X = [(g_data[j][k]**2.) / (g_data[j][j] * g_data[k][k])
             for j, k in itertools.product(xrange(sh[0]), xrange(sh[1]))]
...
def geom_stdev(X_in, r_type='dict'):
    ...
    #
    if r_type.lower() == 'dict':
        return {'mu_g': mu_g, 'sigma_g': sigma_g}
    if r_type.lower() == 'list':
        return [mu_g, sigma_g]
    if r_type.lower() is None:
        return sigma_g
