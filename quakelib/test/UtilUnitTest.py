...
class TestVectors(unittest.TestCase):
    ...
    def test_dot_prod(self):
        ...
        self.assertEqual(
            self.y.dot_product(
                self.x), self.x.dot_product(
                self.y))

    # Ensure cross product is correct and raises an exception for non-3D
    # vectors
    ...
    # Ensure that the object representation can be evaluated to the original
    # object
...
class TestConversion(unittest.TestCase):
    ...
    def test_unit_conversion(self):
        ...
        # changed to almostEqual since 32-bit floating point can't maintain
        # enough precision
        self.assertAlmostEqual(c.year2sec(c.sec2year(1)), 1)
