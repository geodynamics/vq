...
class TestQuakeLibEQSimMetadata(unittest.TestCase):
    ...
    def testExceptionRaising(self):
        record_types = [
            quakelib.META_COMMENT,
            quakelib.META_SIGNATURE,
            quakelib.META_INFO,
            quakelib.META_TITLE,
            quakelib.META_AUTHOR,
            quakelib.META_DATE]
        bad_char_list = ["\r", "\n"]
        ...
        for rec_type in record_types:
            ...
            self.assertRaises(
                IndexError,
                md_file.meta_set_record,
                rec_type,
                1,
                "empty")
            self.assertRaises(
                IndexError,
                md_file.meta_erase_record,
                rec_type,
                1)
    ...
    def testWriteRead(self):
        ...
        self.assertEqual(
            md_file_in.meta_get_record(
                quakelib.META_SIGNATURE, 0), "Sig_3")
        ...
        self.assertRaises(
            ...
            0)
        self.assertEqual(
            md_file_in.meta_num_records(
                quakelib.META_SIGNATURE), 0)

        self.assertEqual(
            md_file_in.meta_get_record(
                quakelib.META_INFO, 0), "Info 1")
        self.assertEqual(
            md_file_in.meta_get_record(
                quakelib.META_INFO,
                1),
