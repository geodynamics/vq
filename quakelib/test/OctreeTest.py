...
class TestRectBound(unittest.TestCase):
    ...
    def testAddMultiRegular(self):
        ...
        quarter_step_vec = quakelib.Vec3(
            quarter_step, quarter_step, quarter_step)
        vec00 = quakelib.Vec3(0, 0, 0)
        vec10 = quakelib.Vec3(1, 1, 1)
        rb = quakelib.RectBound3(vec00, vec10)
        octree = quakelib.Octree3(rb)
        pt_list = [
            [
                ...
                z *
                num_dim_pts *
                num_dim_pts] for x in range(num_dim_pts) for y in range(num_dim_pts) for z in range(num_dim_pts)]
        for pt in pt_list:
        ...
        for pt in pt_list:
            self.assertEqual(
                octree.get_leaf_containing_point(
                    pt[0]).id(), pt[1])
        # And that slightly offset points return the same id
        for pt in pt_list:
            self.assertEqual(
                octree.get_leaf_containing_point(
                    pt[0] + quarter_step_vec).id(), pt[1])
        for pt in pt_list:
            self.assertEqual(
                octree.get_leaf_containing_point(
                    pt[0] - quarter_step_vec).id(), pt[1])
    ...
    def testAddMultiRandom(self):
        ...
        rb = quakelib.RectBound3(vec00, vec10)
        octree = quakelib.Octree3(rb)
        pt_list = [[quakelib.Vec3(random.uniform(-1,
                                                 1),
                                  random.uniform(-1,
                                                 1),
                                  random.uniform(-1,
    ...
    def testAddMultiBigRange(self):
        ...
        rb = quakelib.RectBound3(vec00, vec10)
        octree = quakelib.Octree3(rb)
        pt_list = [[quakelib.Vec3(random.uniform(-DBL_MAX,
                                                 DBL_MAX),
                                  random.uniform(-DBL_MAX,
                                                 DBL_MAX),
                                  random.uniform(-DBL_MAX,
                                                 DBL_MAX)),
