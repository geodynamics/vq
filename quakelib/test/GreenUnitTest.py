...
class TestGreenFunctionCalc(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.slip_vec = quakelib.Vec3(
            math.sin(
                self.slip_rad), math.cos(
                self.slip_rad), 0)
...
# Note that the location can't be too close or we trigger the boundary
# case and the results don't fit a curve
    ...
    # at (x, y) should be equivalent to that at (-x, -y) with a change in sign
    # of Z
    def testGreenSymmetricDisplacement(self):
        ...
        for x in range(-2, 8):
            for y in range(-2, 8):
                ...
                orig_disp = self.ok.calc_displacement_vector(
                    orig_loc,
                    source_dim[2],
                    self.dip_rad,
                    source_dim[0],
                    ...
                    1,
                    1)
                mirror_disp = self.ok.calc_displacement_vector(
                    mirror_loc,
                    source_dim[2],
                    self.dip_rad,
                    source_dim[0],
