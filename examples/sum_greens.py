...
if len(sys.argv) != 4:
    print(
        sys.argv[0] +
        " file_name expected_normal_value expected_shear_value")
    exit(1)

# yoder:
# note that the default input shear/normal greens values (as per the
# specific greens tests built by Eric H.) are: 6.9056016275796917e-08
# -91753588.690448046
...
with h5py.File(file_name, "r") as fp:
    # copy directly to arrays using [()] syntax.
...
# yoder: for small values, this error test is not very good. instead,
# let's look for an explicitly geometric error (aka, e=a/b).
