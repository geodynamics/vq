...
def check_self_consistent(events):
    ...
    for event in events:
        ...
        for elem_id in elements:
            element_sweep_slip_sums[elem_id] = 0
        summed_moment = 0
        ...
        for elem_num in elements:
            ...
            summed_moment += element_sweep_slip_sums[elem_num] * \
                element_area[elem_num] * element_mu[elem_num]
        ...
        for elem_num in total_slips:
            if total_slips[elem_num] != element_sweep_slip_sums[elem_num]:
                print(
                    "ERROR: Total slip not equal to summed sweep slip for event",
                    event.event_num,
                    "element",
                    elem_num)
                error = True
        ...
        if (summed_moment <= 0):
            ...
            if abs(event.getMagnitude() - summed_mag) > 1e-5:
                print(
                    "ERROR: Recorded magnitude and summed sweep magnitude is not equal for event",
                    event.event_num)
                error = True
...
def calc_mean_interevent(events):
    ...
    return sum([event_years[i + 1] - event_years[i]
                for i in range(len(event_years) - 1)]) / (len(event_years) - 1)


def calc_b_val(events):
    mags = [events.event_list[enum].magnitude for enum in events.event_list if events.event_list[
        enum].magnitude < 10 and events.event_list[enum].magnitude > 0]
    ...
    for i in range(10):
        ...
        n_above_mag = math.log10(
            sum(1 for m in mags if m >= cur_mag) / float(len(mags)))
...
if __name__ == "__main__":
    ...
    err = False
    if args.check_consistent and check_self_consistent(events):
        err = True
    ...
    if args.mean_interevent:
        ...
        reldiff = abs(mean_interevent - expected_mean_interevent) / \
            expected_mean_interevent
        if reldiff > 0.02:
            err = True
        print(
